...
export createChatBot = ({
  ...
  buildContext =  ({sessionId, history = undefined, limit = 15}) ->
    ...
    contextIterator = (context, message, i, history) ->
      switch
        when message.chatRole is 'system'
          [context..., new SystemMessage message.text]
        when message.chatRole is 'user'
          # if last message in context is a HumanMessage, add an AIMessage with content '[placeholder]'
          if context[-1]?.constructor.name is 'HumanMessage'
            [context..., (new AIMessage '[empty Message]'), (new HumanMessage message.text)]
          else
            [context..., new HumanMessage message.text]
        when message.chatRole is 'function'
          return context if i is 0 or history[i - 1].chatRole isnt 'assistant'
          [context..., new ToolMessage {content: message.text, tool_call_id: message.tool_id}]
        when message.chatRole is 'assistant'
          return context if i is 0 # skip welcome (anthropic dies if user isnt first in history)
          return context if i is history.length - 1 # last message may not be AIMessage
          # if last message in context is an AIMessage, add an empty HumanMessage.
          if context[-1]?.constructor.name is 'AIMessage'
            [context..., (new HumanMessage '[empty Message]'),
            new AIMessage {content: message.text, tool_calls: message.tools}]
          else
            [context..., new AIMessage {content: message.text, tool_calls: message.tools}]
        else
  ...
  ###*
    ...
    @param {Number} [options.followDelay=1] - set createdAt to followMessageId + followDelay in ms
  ...
  call = ({sessionId, messageStubId, context}) ->
    ...
    .then (response) ->
      ...
      usage =
        ...
        prompt: pt = response?.usage_metadata?.input_tokens ? 0
        completion: ct = response?.usage_metadata?.output_tokens ? 0
        total: pt + ct
      ...
      if toolCalls? and toolCalls.length
        ...
        newMessageId = await createMessageStub {sessionId, text: '...', followMessageId: messageStubId, followDelay: 2}
